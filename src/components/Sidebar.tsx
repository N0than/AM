      ...
      <nav className="space-y-2">
        {menuItems.map((item) => (
          <Link
            key={item.path}
            to={item.path}
            className="flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-gray-700 dark:hover:bg-gray-200 transition-colors"
          >
            <item.icon className="w-5 h-5 text-gray-100 dark:text-gray-900" />
            <span className="text-gray-100 dark:text-gray-900">{item.label}</span>
          </Link>
        ))}
      </nav>
    </aside>
